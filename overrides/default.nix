  ...
  lib.composeManyExtensions [
      ...
      final: prev:
        lib.mapAttrs (_: v:
          addBuildSystem' {
            ...
            drv = v;
            attr = "poetry";
          }) (lib.filterAttrs (n: _: lib.strings.hasPrefix "nixops" n) prev)
        // {
          # NixOps >=2 dependency
          nixos-modules-contrib = addBuildSystem' {
            inherit final;
            drv = prev.nixos-modules-contrib;
            attr = "poetry";
          };
        }
    )
    ...
    (
      ...
      in {
        ...
        #### begin auto sort
        # blosc = prev.blosc.overridePythonAttrs (old: {
        #   nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake];
        # });

        # blosc2 = prev.blosc2.overridePythonAttrs (old: {
        #   nativeBuildInputs = (old.nativeBuildInputs or []) ++ [pkgs.cmake];
        ...
        # clang-tidy = prev.clang-tidy.overridePythonAttrs (old: {
        #   nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.llvm pkgs.ninja];
        #   dontUseCmakeConfigure = true;
        # });

        # frida = prev.frida.overridePythonAttrs (old: let
        #   getFridaHash = version:
        #     {
        #       "16.3.3" = "sha256-WBy4oXPDzgm2644c1Pn0w9lJTgDTzx1kMaAXR0nzyD4=";
        #     }
        #     .${version}
        #     or (
        #       lib.warn "Unknown frida version: '${version}'. Please update the override." lib.fakeHash
        #     );
        #   devkit = pkgs.fetchurl {
        #     url = "https://github.com/frida/frida/releases/download/${old.version}/frida-core-devkit-${old.version}-linux-x86_64.tar.xz";
        #     hash = getFridaHash old.version;
        #   };
        # in {
        #   postPatch = ''
        #     mkdir assets
        ...
        #     popd
        #   '';
        #   nativeBuildInputs = [pkgs.which pkgs.meson];
        # });

        # mkl-random = prev.mkl-random.overridePythonAttrs
        # (old: {
        #   buildInputs = old.buildInputs or [] ++ [pkgs.pkgs.opencl-headers];
        # });

        # pendulum = prev.pendulum.overridePythonAttrs (old: let
        #   unpacked_src = pkgs.runCommand "unpack-pendulum-cargo.lock" {} ''
        #     mkdir $out
        #     tar xf ${old.src}
        #     cp ${old.pname}-${old.version}/rust/Cargo.lock $out/
        #   '';
        # in {
        #   cargoDeps = pkgs.rustPlatform.importCargoLock {
        ...
        #   ''; # make some validation check happy...
        ...
        # pypdfium2 = prev.pypdfium2.overridePythonAttrs (old: { # needs wrapping a ton of things it pulls in by download.
        #   postPatch  = ''
        #   substituteInPlace setupsrc/pypdfium2_setup/packaging_base.py --replace-fail 'PdfiumVer.get_latest()' '"${old.version}"'
        #   '';
        # });

        # pyslurm = prev.pyslurm.overridePythonAttrs (old: {

        #   buildInputs = old.buildInputs or [] ++ [pkgs.slurm];
        #   env = {
        #     SLURM_LIB_DIR = "${lib.getLib pkgs.slurm}/lib";
        #     SLURM_INCLUDE_DIR = "${lib.getDev pkgs.slurm}/include";
        #   };
        # });

        # rtmixer = prev.rtmixer.overridePythonAttrs (old: {
        ...
        # scikit-umfpack = prev.scikit-umfpack.overridePythonAttrs (old: {
        #     nativeBuildInputs = old.nativeBuildInputs or [] ++ [swig pkgs.openblasCompat pkgs.cmake pkgs.pkg-config];
        #     buildInputs = [pkgs.openblasCompat pkgs.suitesparse];
        #     dontUseCmakeConfigure = true;
        # });
        ...
        # trytond-product = prev.trytond-product.overridePythonAttrs (old: {
        #   postPatch = ''
        #     substituteInPlace setup.py --replace-fail "readfp" "read"
        #   '';
        # });

        # uv = prev.uv.overridePythonAttrs (standardMaturin {
        ...
        afdko = prev.afdko.overridePythonAttrs (_old: {
          inherit (pkgs.python3.pkgs.afdko) patches postPatch NIX_CFLAGS_COMPILE dontConfigure FORCE_SYSTEM_LIBXML2;
          nativeBuildInputs =
            prev.afdko.nativeBuildInputs
            or []
            ++ [
              pkgs.cmake
              pkgs.ninja
        ...
        apexpy = prev.apexpy.overridePythonAttrs (old: {
        ...
        aws-adfs = prev.aws-adfs.overrideAttrs (old: {
        ...
        based58 = prev.based58.overridePythonAttrs (standardMaturin {});
        ...
        cattrs = prev.cattrs.overridePythonAttrs (
          old:
            lib.optionalAttrs (old.version == "1.10.0") {
        ...
        chia-rs = prev.chia-rs.overridePythonAttrs (standardMaturin {
          furtherArgs = {
            #  cargoRoot = "wheel";
        ...
        cmlibs-zinc = prev.cmlibs-zinc.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.libxml2.dev];
          buildInputs = (old.buildInputs or []) ++ [pkgs.libxml2 pkgs.xz pkgs.freetype pkgs.libGLU];
        });

        cmsis-pack-manager = prev.cmsis-pack-manager.overridePythonAttrs (standardMaturin {});
        cramjam =
        ...
        correctionlib = prev.correctionlib.overridePythonAttrs (_old: {
          nativeBuildInputs = prev.correctionlib.nativeBuildInputs or [] ++ [pkgs.cmake];
          buildInputs = prev.correctionlib.buildInputs or [] ++ [pkgs.zlib];
          dontUseCmakeConfigure = true;
        });

        cryptg = prev.cryptg.overridePythonAttrs (standardMaturin {});
        ...
        css-inline = prev.css-inline.overridePythonAttrs (standardMaturin {});
        ...
        cffi =
          ...
          if final.python.implementation == "pypy"
          then null
          else
            (
        ...
        dbt-extractor =
          prev.dbt-extractor.overridePythonAttrs
          (
            standardMaturin {
              outputHashes = {
        ...
        deptry = prev.deptry.overridePythonAttrs (standardMaturin {});
        ...
        dm-tree = prev.dm-tree.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake final.pybind11];
          dontUseCmakeConfigure = true;
        });

        docutils = prev.docutils.overridePythonAttrs (
          _:
            lib.optionalAttrs (lib.versionOlder prev.docutils.version "0.16" && lib.versionAtLeast prev.setuptools.version "60") {
        ...
        esig = prev.esig.overridePythonAttrs (old: {
        ...
        fast-query-parsers = prev.fast-query-parsers.overridePythonAttrs (standardMaturin {});
        ...
        ffcv = prev.ffcv.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.pkg-config];
          buildInputs = old.buildInputs or [] ++ [pkgs.opencv pkgs.libjpeg];
        });
        ...
        george = prev.george.override {
          # pypi source seems to be missing cpp sources?
          preferWheel = true;
        };

        gilknocker = prev.gilknocker.overridePythonAttrs (standardMaturin {});
        ...
        granian = prev.granian.overridePythonAttrs (old:
          lib.optionalAttrs (!(old.src.isWheel or false))
          (
            let
              githubHash =
        ...
        hf-transfer = prev.hf-transfer.overridePythonAttrs (standardMaturin {});
        ...
        implicit = prev.implicit.overridePythonAttrs (old: {
        ...
        ipydatawidgets = prev.ipydatawidgets.overridePythonAttrs (
          old: {
        ...
        jellyfish = prev.jellyfish.overridePythonAttrs (standardMaturin {});
        ...
        jiter = prev.jiter.overridePythonAttrs (standardMaturin {});
        ...
        jsii = prev.jsii.overridePythonAttrs (old:
          lib.optionalAttrs (!(old.src.isWheel or false)) {
            postPatch = ''
              substituteInPlace pyproject.toml \
        ...
        jsonschema =
          ...
          (old:
            lib.optionalAttrs (lib.versionAtLeast old.version "4.0.0") {
        ...
        libcst = prev.libcst.overridePythonAttrs (standardMaturin {
          furtherArgs = {
            cargoRoot = "native";
            buildAndTestSubdir = "native/libcst";
          };
        });
        ...
        lintrunner = prev.lintrunner.overridePythonAttrs (standardMaturin {});
        ...
        lsassy = prev.lsassy.overridePythonAttrs (
          old:
            lib.optionalAttrs (old.version == "3.1.1") {
        ...
        lzallright = prev.lzallright.overridePythonAttrs (standardMaturin {});
        ...
        mbf-gtf = prev.mbf-gtf.overridePythonAttrs (standardMaturin {});
        ...
        minify-html = prev.minify-html.overridePythonAttrs (standardMaturin {});

        minijinja = prev.minijinja.overridePythonAttrs (standardMaturin {});
        ...
        mitmproxy-rs = prev.mitmproxy-rs.overridePythonAttrs (offlineMaturin {});

        mitmproxy-wireguard = prev.mitmproxy-wireguard.overridePythonAttrs (standardMaturin {});
        ...
        molecule = prev.molecule.overridePythonAttrs (
          old:
            lib.optionalAttrs (lib.versionOlder old.version "3.0.0") {
              patches =
                old.patches
                or []
                ++ [
        ...
        mwxml = prev.mwxml.overridePythonAttrs (old: {
          propagatedBuildInputs =
            removePackagesByName
            (old.propagatedBuildInputs or [])
            (lib.optionals (final ? mwcli) [final.mwcli]);
        });
          ...
          in
            ...
            // lib.optionalAttrs (old.format != "wheel") {
              ...
              patches =
                old.patches
                or []
                ++ lib.optionals (lib.versionAtLeast old.version "0.900" && lib.versionOlder old.version "0.940") [
                  (pkgs.fetchpatch {
        ...
        in
          prev.nbconvert.overridePythonAttrs (old: {
            postPatch = lib.optionalString (!(old.src.isWheel or false)) (
              patchExporters
              + lib.optionalString (lib.versionAtLeast final.nbconvert.version "7.0") ''
                substituteInPlace \
                  ./hatch_build.py \
                  --replace \
        ...
        nh3 = prev.nh3.overridePythonAttrs (standardMaturin {});

        notebook = prev.notebook.overridePythonAttrs (
          old:
            lib.optionalAttrs (lib.versionAtLeast old.version "7.0.0") {
              buildInputs =
                old.buildInputs
                or []
                ++ [
        ...
        numpy = prev.numpy.overridePythonAttrs (
          ...
          in {
            ...
            format =
              if ((old.format or null) == "poetry2nix")
              then
                (
                  if lib.versionAtLeast prev.numpy.version "2.0.0"
                  then "pyproject"
                  else "setuptools"
                )
              else old.format or null;
        ...
        opencv-contrib-python-headless = prev.opencv-contrib-python-headless.overridePythonAttrs final._opencv-python-override;
        ...
        openlineage-sql = prev.openlineage-sql.overridePythonAttrs (standardMaturin {});
        ...
        orjson = prev.orjson.overridePythonAttrs (standardMaturin {
          furtherArgs = {
            buildInputs = prev.orjson.buildInputs or [] ++ lib.optionals pkgs.stdenv.isDarwin [pkgs.libiconv];
          };
        });
        ...
        patchelf = prev.patchelf.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.autoconf pkgs.automake];
          dontUseCmakeConfigure = true;
        });

        pathpy = prev.pathpy.overridePythonAttrs (old: {
          preBuild = ''
            touch /build/pathpy-${old.version}/HISTORY.rst
          '';
        });
        ...
        pdoc-pyo3-sample-library = prev.pdoc-pyo3-sample-library.overridePythonAttrs (standardMaturin {});
        ...
        pillow = prev.pillow.overridePythonAttrs (
          ...
          in {
            ...
            patches =
              (old.patches or [])
              ++ pkgs.lib.optionals (!(old.src.isWheel or false) && old.version == "9.5.0") [
                (pkgs.fetchpatch {
        ...
        pillow-jpls = prev.pillow-jpls.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake pkgs.ninja pkgs.charls pkgs.fmt pkgs.eigen];
          env.SETUPTOOLS_SCM_PRETEND_VERSION = old.version;
          ...
          '';
          env.CMAKE_ARGS = "--preset=sysdeps";
          CMAKE_PREFIX_PATH = "${pkgs.charls}/lib/cmake/charls:${pkgs.fmt.dev}/lib/cmake/fmt:${pkgs.eigen}/share/eigen3/cmake";
        ...
        polars-lts-cpu = prev.polars-lts-cpu.overridePythonAttrs (standardMaturin {});
        ...
        primecountpy = prev.primecountpy.overridePythonAttrs (old: {
          buildInputs = old.buildInputs or [] ++ [pkgs.primecount];
        });
        ...
        publicsuffix2 = prev.publicsuffix2.overridePythonAttrs (old: {
          buildInputs = old.buildInputs or [] ++ [final.requests];
          propagatedBuildInputs = old.propagatedBuildInputs or [] ++ [final.requests];
          dontUseCmakeConfigure = true;
        });

        py-bip39-bindings = prev.py-bip39-bindings.overridePythonAttrs (standardMaturin {});

        py-ed25519-zebra-bindings = prev.py-ed25519-zebra-bindings.overridePythonAttrs (standardMaturin {});
        ...
        pyarrow = prev.pyarrow.overridePythonAttrs (
          old:
            lib.optionalAttrs ((!old.src.isWheel or false) && lib.versionAtLeast old.version "0.16.0")
            (
              let
        ...
        pycddl = prev.pycddl.overridePythonAttrs (standardMaturin {});
        ...
        pycrdt = prev.pycrdt.overridePythonAttrs (standardMaturin {});
        ...
        pyee = prev.pyee.overrideAttrs (
          old:
            lib.optionalAttrs (lib.versionOlder old.version "11.1.0") {
              postPatch =
                (old.postPatch
                  or "")
                + lib.optional (lib.versionOlder old.version "11.1.0")
                ''
                  sed -i setup.py \
                    -e '/setup_requires/,/],/d' \
        ...
        pyheck = prev.pyheck.overridePythonAttrs (standardMaturin {});

        pyjnius = prev.pyjnius.overridePythonAttrs (
          old: {
            nativeBuildInputs = old.nativeBuildInputs or [] ++ [jdk];
          }
        );

        pykcs11 = prev.pykcs11.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.swig];
          buildInputs = old.buildInputs or [] ++ [pkgs.swig];
        });
        ...
        pyodbc = prev.pyodbc.overridePythonAttrs (
          old:
            ...
            then {
              ...
              nativeBuildInputs = (old.nativeBuildInputs or []) ++ [pkgs.unixODBC];
              buildInputs = (old.builtInputs or []) ++ [pkgs.unixODBC];
              NIX_CFLAGS_COMPILE = "-I ${lib.getDev pkgs.unixODBC}/include/unixODBC";
            }
        );
        ...
        pyorc = prev.pyorc.overridePythonAttrs (
          old: let
            apache_orc = let
              ver = "1.9.1";
              url = "https://archive.apache.org/dist/orc/";
            in
              builtins.fetchurl {
                url = "${url}orc-${ver}/orc-${ver}.tar.gz";
                sha256 = "sha256:09hn01yy72yyy00946rqb7b2hf2ghvwhpqcl358gj5i5av394i40";
              };
          in {
            dontUseCmakeConfigure = true;
            nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake pkgs.orc pkgs.protobuf pkgs.lz4 pkgs.zstd pkgs.snappy pkgs.zlib pkgs.snappy.dev];
            preBuild = ''
              mkdir deps
              cd deps && tar xf ${apache_orc}
              cd ../
            '';
          }
        );

        pyperscan = prev.pyperscan.overridePythonAttrs (standardMaturin {});
        ...
        pypolyagamma = prev.pypolyagamma.overridePythonAttrs (_old: {
          preBuild = ''
            ...
            cd ../../
            mv deps/gsl/gsl*/* deps/gsl/
            patchShebangs deps/gsl/gsl/configure
          '';
        });
        ...
        pyqt6 = let
          ...
          pyqt6 = pyqt6-wheel.overridePythonAttrs (old: let
        ...
        pyreqwest-impersonate = prev.pyreqwest-impersonate.overridePythonAttrs (standardMaturin {
          outputHashes = {
            # todo: make version dependend
            "reqwest-impersonate-0.11.84" = "sha256-2J9fKNdB+j/usd+oWgrvXfWOQoh9Oh/qpmbbLe6iwjA=";
          };
          furtherArgs = {
        ...
        pyrevm = prev.pyrevm.overridePythonAttrs (standardMaturin {});

        pyrfr = prev.pyrfr.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [swig];
        });

        pyscard = prev.pyscard.overridePythonAttrs (
          old:
        ...
        pysequoia = prev.pysequoia.overridePythonAttrs (standardMaturin {});

        pyside6-addons = prev.pyside6-addons.overridePythonAttrs (old:
        ...
        pytextspan = prev.pytextspan.overridePythonAttrs (standardMaturin {});
        ...
        python-rtmidi = prev.python-rtmidi.overridePythonAttrs (old: {
          nativeBuildInputs = (old.nativeBuildInputs or []) ++ [pkgs.pkg-config];
          buildInputs = (old.buildInputs or []) ++ [pkgs.libjack2 pkgs.alsa-lib];
        });
        ...
        pytokenizations = prev.pytokenizations.overridePythonAttrs (standardMaturin {});
        ...
        pywinpty = prev.pywinpty.overridePythonAttrs (standardMaturin {});

        pyxel = prev.pyxel.overridePythonAttrs (
          old: {
            buildInputs =
              [
                pkgs.SDL2
        ...
        pyxirr = prev.pyxirr.overridePythonAttrs (standardMaturin {});
        ...
        qh3 = prev.qh3.overridePythonAttrs (standardMaturin {});

        quil = prev.quil.overridePythonAttrs (standardMaturin {});
        ...
        regress = prev.regress.overridePythonAttrs (standardMaturin {});
        ...
        rtoml = prev.rtoml.overridePythonAttrs (standardMaturin {});
        ...
        ruff = let
          getOutputHashes = version:
            ...
            .${version}
            or (
              lib.warn "Unknown ruff version: '${version}'. If the build fails because of missing outputHashes for git dependencies, please update poetry2nix/overrides/default.nix." {}
            );
          #sha256 = getRepoHash prev.ruff.version;
        in
          prev.ruff.overridePythonAttrs (old: let
            cargoLock = "${extractCargoLock old.src}/Cargo.lock";
          in
            # let
            #     src = pkgs.fetchFromGitHub {
            #       owner = "astral-sh";
            ...
            lib.optionalAttrs (!(old.src.isWheel or false)) {
              ...
              cargoDeps = pkgs.rustPlatform.importCargoLock {
                lockFile = cargoLock;
                outputHashes = getOutputHashes prev.ruff.version;
              };
        ...
        scikit-misc = prev.scikit-misc.overridePythonAttrs (
          old: {
            inherit (pkgs.python3.pkgs.scikit-misc) postPatch;
            nativeBuildInputs = (old.nativeBuildInputs or []) ++ [pkgs.gfortran pkgs.git pkgs.pkg-config];
            buildInputs = old.buildInputs or [] ++ [pkgs.openblasCompat pkgs.lapack];
          }
        );
        ...
        scs = prev.scs.overridePythonAttrs (old: {
          inherit (pkgs.python3.pkgs.scs) buildInputs;
          nativeBuildInputs =
            (old.nativeBuildInputs or [])
            ++ [
              prev.meson-python
        ...
        uwsgi =
          prev.uwsgi.overridePythonAttrs
          (old:
            {
              buildInputs = old.buildInputs or [] ++ [pkgs.ncurses];
            }
        ...
        shazamio-core = prev.shazamio-core.overridePythonAttrs (standardMaturin {});
        ...
        silva-core-conf = prev.silva-core-conf.overridePythonAttrs (old: {
          propagatedBuildInputs =
            removePackagesByName
            (old.propagatedBuildInputs or [])
            (lib.optionals (final ? silva-core-services) [final.silva-core-services]);
        });

        solders = prev.solders.overridePythonAttrs (standardMaturin {});
        ...
        sparse-dot-topn = prev.sparse-dot-topn.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake pkgs.ninja];
          CMAKE_PREFIX_PATH = "${prev.nanobind}/lib/python${lib.versions.majorMinor final.python.version}/site-packages/nanobind/cmake";
          dontUseCmakeConfigure = true;
        });

        spglib = prev.spglib.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.cmake pkgs.ninja];
          dontUseCmakeConfigure = true;
        });
        ...
        sqlglotrs = prev.sqlglotrs.overridePythonAttrs (standardMaturin {});
        ...
        swig = prev.swig.overridePythonAttrs (old: {
          nativeBuildInputs = (old.nativeBuildInputs or []) ++ [pkgs.swig pkgs.ninja pkgs.bison pkgs.pcre2 pkgs.pcre2.dev];
          dontUseCmakeConfigure = true;
        });
        ...
        tensorstore = prev.tensorstore.override {
          # at least in 1.63 the uploaded source tar is broken?
          preferWheel = true;
        };
        ...
        tiktoken = prev.tiktoken.overridePythonAttrs (standardMaturin {
          furtherArgs = let
            cargo_lock = ./. + "/tiktoken/${prev.tiktoken.version}-Cargo.lock";
          in {
            cargoDeps = pkgs.rustPlatform.importCargoLock {
              # tiktoken does not come with Cargo.lock
              # so we supply one of our own.
              lockFile = cargo_lock;
            };
            postPatch = ''
              cp ${cargo_lock} /build/tiktoken-${prev.tiktoken.version}/Cargo.lock
            '';
          };
        });
        ...
        tlparse = prev.tlparse.overridePythonAttrs (standardMaturin {});

        tokenizers = prev.tokenizers.overridePythonAttrs (standardMaturin {
          furtherArgs = {
        ...
        topgrade = prev.topgrade.overridePythonAttrs (standardMaturin {});
        ...
        triton = prev.triton.overridePythonAttrs (old: {
          propagatedBuildInputs =
            (removePackagesByName (old.propagatedBuildInputs or []) [final.torch])
            ++ [
        ...
        turibolt = prev.turibolt.overridePythonAttrs (old: {
          buildInputs = old.builtInputs or [] ++ [prev.requests];
        });

        tzfpy = prev.tzfpy.overridePythonAttrs (standardMaturin {});
        ...
        uuid-utils = prev.uuid-utils.overridePythonAttrs (standardMaturin {});

        uv = prev.uv.overridePythonAttrs (standardMaturin {});
        #fschat = prev.fschat.overridePythonAttrs (standardMaturin {});
        ...
        vhacdx = prev.vhacdx.overridePythonAttrs (_old: let
          vhacd = pkgs.fetchFromGitHub {
            owner = "kmammou";
            repo = "v-hacd";
            rev = "ab01cba71cc563b73bd3f0d574d346b2b797ea4b";
            sha256 = "sha256-NGNsqkXuhusWU5He37kGyCjfyefmOEt+Cl4749GIDqk=";
          };
        in {
          preBuild = ''
            cp -r ${vhacd}/include/* src/vhacdx
          '';
        });
        ...
        in
          prev.watchfiles.overridePythonAttrs (old: let
            src = pkgs.fetchFromGitHub {
              owner = "samuelcolvin";
              repo = "watchfiles";
              rev = "v${old.version}";
            ...
            cargoDeps = let
              hash = getCargoHash prev.watchfiles.version;
            in
              if hash == null
              then
        ...
        wxpython = prev.wxpython.overridePythonAttrs (old: let
