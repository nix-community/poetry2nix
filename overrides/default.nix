  ...
  lib.composeManyExtensions [
    ...
    (
      ...
      in {
        ...
        aiologger = prev.aiologger.overridePythonAttrs (old: {
        ...
        asynclog = prev.asynclog.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools];
        });
        ...
        docutils = prev.docutils.overridePythonAttrs (
          old: {nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.flit-core];}
        );
        ...
        flake8-annotations-complexity = prev.flake8-annotations-complexity.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools];
        });
        flake8-builtins = prev.flake8-builtins.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.hatchling];
        });
        flake8-executable = prev.flake8-executable.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools final.setuptools-scm];
        });
        flake8-pep3101 = prev.flake8-pep3101.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.hatchling];
        });
        flake8-rst-docstrings = prev.flake8-rst-docstrings.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools];
        });
        ...
        hdwallet = prev.hdwallet.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools];
        });
        ...
        hishel = prev.hishel.overridePythonAttrs (old: {
          build-system = [
            final.hatch-fancy-pypi-readme
            final.hatchling
          ];
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools final.hatchling final.setuptools-scm];
        });
        ...
        kubernetes-asyncio = prev.kubernetes-asyncio.overridePythonAttrs (old: {
        ...
        pyfa-converter-v2 = prev.pyfa-converter-v2.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.poetry-core];
        });
        ...
        in
          prev.ruff.overridePythonAttrs (old: let
            src = pkgs.fetchFromGitHub {
              ...
              repo = "ruff";
              rev =
                if (lib.versionOlder old.version "0.5.0")
                then "v${old.version}"
                else old.version;
              inherit sha256;
            };
            ...
            in
              if (hash == null || builtins.isAttrs hash)
              then
        ...
        sqlmodel = prev.sqlmodel.overridePythonAttrs (old: {
          ...
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [pkgs.pdm final.pdm-backend];
          patches = [
            (pkgs.fetchpatch {
        ...
        watchfiles = let
          ...
          getRepoHash = version:
            {
              "0.24.0" = "sha256-kFScg3pkOD0gASRtfXSfwZxyW/XvW9x0zgMn0AQek4A=";
          ...
          getCargoHash = version:
            {
              "0.24.0" = "sha256-m7XFpbujWFmDNSDydY3ec6b+AGgrfo3+TTbRN7te8bY=";
        ...
        uuid = prev.uuid.overridePythonAttrs (old: {
          nativeBuildInputs = old.nativeBuildInputs or [] ++ [final.setuptools];
        });
