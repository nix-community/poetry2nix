...
let
  ...
  poetryLib = import ./lib.nix {
    inherit lib pkgs pyproject-nix;
    inherit (pkgs) stdenv;
  };
        ...
        let
          supportsPythonVersion =
            ...
            if pkgMeta ? marker then
              ...
              true && poetryLib.checkPythonVersions pyVersion pkgMeta.python-versions;
        in
        ...
        [
          (
            ...
            lib.attrsets.mapAttrs (
              name: value:
              if lib.isDerivation value && final.hasPythonModule value && (normalizePackageName name) != name then
                null
              else
                value
            ) prev
  ...
  */
  mkPoetryEnv =
    ...
    let
      ...
      getEditableDeps =
        set:
        lib.mapAttrs (_name: value: projectDir + "/${value.path}") (
