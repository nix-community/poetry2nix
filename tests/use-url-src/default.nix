...
let
  ...
  is_wheel_attr_test =
    x: lib.warnIf (!stdenv.isLinux && url_is_wheel) "url should resolve to a not wheel" x;
  is_wheel_test =
    x:
    assert lib.strings.hasSuffix "tar.gz" url_nix_store;
    x;
  app_builds =
    x:
    assert lib.isDerivation app;
    x;
in
lib.pipe app [
  is_wheel_attr_test
